...
To get a good baseline for your daily energy needs you can use the following formula[^2]: 
$$ \sum_{1 \leq t \leq 24} c^t_{out} = \begin{cases}
 10 \cdot \text{ weight (kg)} + 6.25 x \text{height (cm)} - 5 x \text{age (y)}  \text{ for men}\\ 
10 \cdot \text{ weight (kg)} + 6.25 x \text{height (cm)} - 5 x \text{age (y)} - 161 \text{ for females}
\end{cases}
$$
...
Note that, obviously, if you do exercise at time $t$ you will use more energy at this time. 
...
**Any diet, food claim with the aim of losing weight which does create a negative energy balance is a fraud and should be rejected without further consideration.**

Losing weight is not easy in real life circumstances: After doing the obvious things like changing soda to diet soda/drinking water and eating healthy it is always connected with sacrifice called *diet*. Designing diets in practice is an art and are out of scope for this small post.
